...
export class BattleEffects {
        ...
        events.forEach((event) => {
            ...
            if (event.type === CombatEventType.Damage) {
                this.scene.time.delayedCall(impactDelay, () => {
                    ...
                    this.renderFloatMessage(event.targetUid, `−${event.amount}`, COLORTOKEN.Accent.Red.Hex);
                });
            }
            ...
            if (event.type === CombatEventType.Miss) {
                this.scene.time.delayedCall(impactDelay, () => {
                    this.renderFloatMessage(event.targetUid, 'Промах', COLORTOKEN.Foreground.Quanternary.Hex);
                });
            }
        });
    }
    ...
    private renderFloatMessage(targetId: string, text: string, color: string): void {
        ...
        const label = this.scene.add.text(
            ...
            text,
            {
                ...
                color: COLORTOKEN.Accent.Red.Hex,
    ...
    private playHitImpact(targetId: string): void {
        ...
        tintable.setTint?.(COLORTOKEN.Accent.Red.Numeric);
        ...
        const flash = this.scene.add.circle(combatantSprite.x, flashY, flashSize, COLORTOKEN.Accent.Red.Numeric, 1).setDepth(85);
        this.scene.tweens.add({
            targets: flash,
            alpha: 0,
            scale: 2,
